...
- **`RuleOperationEntity.resource` is Signum's `OperationTypeEmbedded`, not two flat FK fields.** An
  operation rule is keyed by a PAIR (which operation, on which type), and Signum models that pair as an
  embedded — so its columns are `resource_operation_id` / `resource_type_id`, and `resource` is what all
  four rule tables call their resource. altea had flattened it, the reason recorded in the comment being
  that a unique index should not have to walk an embedded; index selectors DO walk embeddeds
  (`accessedFields` resolves a dotted path), so the reason was already gone. **An existing altea database
  needs the sync's RENAME prompts answered** (`operation_id` → `resource_operation_id`, `type_id` →
  `resource_type_id`): the no-rename path drops both columns and adds the new ones defaulted to 0, which
  loses every operation rule and then fails on the foreign key. Aligned beside it: all three
  `Rule*ConditionEntity_Condition` rows gained the `@rowOrder` Signum's `[PreserveOrder]` gives them.
