...
// Port of Southwind's `Public/CatalogController.cs` — the ANONYMOUS catalog the public landing page reads
// (publicApi/PublicCatalog.tsx). Southwind's OTHER catalog controller, the API-key-authenticated one that
...
//    `/api/catalog` PREFIX (an Express `use` covers the prefix itself), so keeping Southwind's path would
...
export namespace PublicCatalogApi {
    ...
    // Southwind's two static PropertyRoutes. Built lazily rather than at module scope: a route resolves
    ...
    export function start(ws: WebBuilder): void {
        ...
        // Signum's `[SignumAllowAnonymous]`: the landing page shows this to a visitor who has never logged
