...
// Port of Southwind's `Public/RegisterUserModel.cs` — the SELF-SERVICE registration form: an anonymous
// visitor fills it in and the app creates an Employee plus the User that logs in as them (see
// PublicLogic.server.ts, the port of Southwind's PublicController).
//
// altea needs no `[AllowUnauthenticated]` counterpart. Signum's attribute opts a type into the reflection
...
// so it reaches an anonymous visitor as it stands. `@reflect` is what Signum's
// `ReflectionServer.RegisterLike(typeof(RegisterUserModel), () => true)` does: register the type at all.
@reflect
export class RegisterUserModel extends ModelEntity {
    ...
    // Signum's `PropertyValidation(pi) => UserEntity.OnValidatePassword(Password, null)`. altea has no such
...
/** Signum's `UserEntity.OnValidatePassword` minimum, mirrored on both tiers (AuthServer enforces the same
 *  number for a password change). */
export const MIN_PASSWORD_LENGTH = 5;

// Southwind's `[AllowUnauthenticated] enum RegisterUserMessage`. A MESSAGE container rather than a reflected
