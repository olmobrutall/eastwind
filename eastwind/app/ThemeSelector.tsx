...
// Port of Southwind's `ThemeSelector.tsx` — the navbar PALETTE picker: any of the bootswatch themes, or the
// default bootstrap. It pairs with @altea/altea's ThemeModeSelector (light / dark / auto): picking a palette
// that is inherently dark dispatches Signum's own `change-theme-mode` event so the mode follows it.
//
// The stylesheet comes off the BOOTSWATCH CDN, as in Southwind — a palette is a full bootstrap build, so
...
//
// Divergences from Southwind, both about WHICH stylesheet gets disabled while a palette is active:
//  - in DEV the base bootstrap arrives as vite's injected <style> for `bootstrap/dist/css/bootstrap.min.css`
//    (eastwind imports the prebuilt CSS in main.client.ts). Southwind looks for its `custom.scss` instead,
//    which is its SCSS entry point — there is no such file here.
//  - in a BUILD nothing is disabled, where Southwind disables its `theme-*.css` link. Southwind compiles its
...
export function ThemeSelector(): React.ReactElement {
    ...
    function select(t: string): void {
        setTheme(t);
        // Tell the mode selector to follow a palette that is inherently dark / light (Signum's own event).
        // Only when the palette HAS an opinion: "Default bootstrap" supports both, so it leaves the mode
        // alone rather than clobbering an explicit choice — Southwind falls back to "light" here, which
